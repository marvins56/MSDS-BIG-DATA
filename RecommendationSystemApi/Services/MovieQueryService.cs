    ...
    public class MovieQueryService : IMovieQueryService
    {
        ...
        public async Task<QueryResultModel> QueryMoviesAsync(MovieQueryModel query)
        {
            ...
            var movies = await _context.Set<MovieMetadata>()
                .Where(m =>
                    m.Title.Contains(query.MovieName) ||
                    m.OriginalTitle.Contains(query.MovieName) ||
                    m.OriginalTitle.Contains(query.Keyword) ||
                    m.Overview.Contains(query.Keyword) ||
                    m.Overview.Contains(query.MovieName) ||
                    m.OriginalTitle.Contains(query.Keyword)
                )
        ...
        public async Task<List<string>> GetDistinctGenres()
        {
            var distinctGenres = await _context.Set<Genre>()
                .Select(g => g.Name)
                .Distinct()
                .ToListAsync();
